...
#front face detctor bgr2gray
#bounding box
#face.left, face.top,face.right,face.bottom
#predictor for facial landmarks from shape detector file(downloaded asset)
#find landmarks number from image(cv2.puttext)
#separate points and crop image with help of points
#masked imagewith polygon instead of rectangle
    ...
    for face in faces:
        ...
        if len(myPoints) != 0:
            try:
                ...
                maskLips = createBox(
                    img, myPoints[48:61], masked=True, cropped=False)
                ...
                imgOriginalGray = cv2.cvtColor(
                    imgOriginalGray, cv2.COLOR_GRAY2BGR)
                imgColorLips = cv2.addWeighted(
                    imgOriginalGray, 1, imgColorLips, 0.4, 0)
